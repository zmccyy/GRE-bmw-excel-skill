...
- **必须追加中文翻译**（仅限 `prefix` 和 `root` 两张表）：
  - 翻译直接**内联到原 Examples 列（D 列）**，**不新增**翻译列
  - 格式：`英文例词 / 中文翻译`
  - 即在原 `,` 分隔的英文例词串后追加 ` / ` 再加翻译串
  - 翻译串内部使用 `,` 分隔每个词条，格式为 `词 词性. 中文释义`（如 `predict v.预测`）
  - 翻译来源：优先使用 **GRE 词汇书**（如 要你命3000 / GRE核心词汇考法精析 / 陈琦再要你命3000），次选 **Merriam-Webster 词典**
  - 找不到的词用 `unclear` 占位，**不要瞎编**
  - **suffix 表和 mythology&history 表不要求加翻译**（按用户 2026-06-28 决定）
- 完整示例：
  - 仅英文：`predict, pretest, preview`
  - 内联翻译后：`predict, pretest, preview / predict v.预测, pretest n.预测测试, preview v.预览`
...
- Examples 字段保持图片中的原始分隔方式（`/` 或 `,`），并按 2.6 节规则内联追加中文翻译（**仅 `prefix` / `root` 表**）
- **绝不要新增 "Examples Translation" 之类的翻译列**——翻译一律内联到原 Examples 列
- **每张图的 OCR 结果都必须经用户确认后再写入**，不允许跳过
- **mythology&history sheet 处理规则已更新**：现在支持处理神话典故模式（见 2.7）。判定方法：右侧无红点行 + 左侧有黑体大写专有名词 + 下方紧跟完整英文故事句

## 当前 Excel 状态（动态更新，2026-06-28）

- `prefix`: 实际有数据的行 3-100，编号 1-98（已确认；Examples 列已内联中文翻译）
- `root`: 实际有数据的行 3-203，编号 1-201（已确认；Examples 列已内联中文翻译）
- `suffix`: 实际有数据的行 3-28，编号 1-26（截至初始化；**不要求**加翻译）
- `mythology&history`: 实际数据行 3-67，编号 1-65（已确认；**不要求**加翻译）

**翻译内联完成情况（2026-06-28）**：
- 翻译源：GRE 词汇书（要你命3000 / GRE核心词汇考法精析）+ Merriam-Webster 词典
- 翻译格式：`英文例词 / 词 词性. 中文释义, 词 词性. 中文释义, ...`
- 范围：仅 `prefix` 和 `root` 两张表
- 0 条未翻译（已通过补全脚本 `_inline2.py` 覆盖所有匹配）
